...
- **Dependency Injection**: Centralized DI container using GetIt (`dependency_injection_container.dart`) with interface-based registration, ensuring all dependencies are resolved from a single source of truth for improved testability and maintainability
...
```
lib/
├── main.dart                    # App entry point, Firebase init, DI init, MultiBlocProvider
│
...
- **Dependency Injection**: Centralized GetIt container (`dependency_injection_container.dart`) with interface-based registration — repositories are registered against abstract types, Cubits as factories, and external services (Firebase, HTTP client) as lazy singletons
...
- **get_it** — Service locator for centralized dependency injection

### Networking & Functional Programming

- **http** (^1.6.0) — HTTP client for API calls (registered as singleton in DI container)
...
## 📂 State Management & Dependency Injection Overview

All dependencies are registered in `lib/core/di/dependency_injection_container.dart` using GetIt. The `initDependencies()` function is called during app startup in `main.dart`, before `runApp()`. Cubits are registered as **factories** (new instance per resolution) while repositories, use cases, and external services are registered as **lazy singletons**.

The following Cubits are resolved via `getIt<T>()` in the root `MultiBlocProvider` in `main.dart`:
